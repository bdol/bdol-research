...
/**
 * Instantiates a matrix with pre-allocated data. There is no guarantee that the
 * matrix will preserve this data, and the matrix will not free the data.
 */
template<class T>
Matrix<T>::Matrix(T* dataPtr, int height, int width, bool rowMajor) {
  this->height = height;
  this->width = width;
  this->rowMajor = rowMajor;

  this->elements = dataPtr;
...
void Matrix<float>::eig(Matrix<float>* V, Matrix<float>* d) {
 ...
 // std::cout << "float!" << std::endl;
  ...
  if (info != 0) {
    std::cout << "Error, ssyev eig returned error code " << info << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
void Matrix<float>::qrInPlace() {
  ...
  info = LAPACKE_sgeqrf(CblasColMajor, 
    ...
    this->h(), tau);
  if (info != 0) {
    std::cout << "Error, sgeqrf in qrInPlace() returned error code " << info << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  if (info != 0) {
    std::cout << "Error, sorgqr in qrInPlace() returned error code " << info << std::endl;
    exit(EXIT_FAILURE);
  }
...
void Matrix<T>::writeToFile(std::string filename) {
  ...
  for (int i=0; i<this->height; i++) {
    for (int j=0; j<this->width; j++) {
      if (this->rowMajor) {
        f << this->elements[this->width*i+j] << " ";
      } else {
        f << this->elements[this->height*j+i] << " ";
      }
    }
    f << "\n";
  }
